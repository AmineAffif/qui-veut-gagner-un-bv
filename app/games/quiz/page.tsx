...
const QuizPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchGame = async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/games/random_game`, {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    if (game && currentQuestionIndex < game.questions.length) {
      setShuffledAnswers(shuffleArray(game.questions[currentQuestionIndex].answers));
    }
    ...
    if (game && currentQuestionIndex >= game.questions.length && user) {
      const saveGame = async () => {
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/games`, {
          ...
          credentials: "include",
          body: JSON.stringify({ user_id: user.id, score, question_ids: game.questions.map(q => q.id) }),
        });
      };
      saveGame();
    }
          ...
          <div className="grid gap-4">
            {shuffledAnswers.map((answer) => (
              <Button
                ...
                variant="outline"
                className={`w-full ${selectedAnswer !== null ? 'pointer-events-none' : ''} ${selectedAnswer === answer.id ? (isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : ''}`}
